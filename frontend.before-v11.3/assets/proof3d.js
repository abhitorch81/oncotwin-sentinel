...
const CASES=[
  ['feature_quality',0,COLORS.rose],['cancer_progression',1,COLORS.mint],['model_drift',2,COLORS.blue],
  ['schema_mutation',3,COLORS.amber],['biomarker_discordance',4,COLORS.purple],['protein_conformation',5,COLORS.blue],
  ['microenvironment_escape',6,COLORS.mint],['ctdna_mrd_rebound',7,COLORS.rose],['bispecific_safety',8,COLORS.amber],
  ['cart_antigen_escape',9,COLORS.purple],['neoantigen_vaccine_drift',10,COLORS.blue],['radiopharmaceutical_mismatch',11,COLORS.mint],
].map(([id,i,color])=>{const angle=i/12*Math.PI*2-Math.PI/2;return {id,position:[Math.cos(angle)*5.1,Math.sin(angle)*3.8,(i%3-1)*.12],color}});
...
function makeWorld(spec,pickMeshes){
  ...
  if(!group.userData.anim)buildFeatureWorld(group,spec.color,pickMeshes);
