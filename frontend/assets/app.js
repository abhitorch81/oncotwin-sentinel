import { initCockroachOps } from './cockroach_ops.js?v=11.4.0';

const $ = id => document.getElementById(id);
const GCP_PROJECT_ID = 'project-1f5f7d56-1029-4c78-a68';
const state = {cohorts:[], cohort:null, twin:null, twinLoading:null, twinConfig:null, proof3d:null, proof3dLoading:null, proofData:null, observatory:null, observatoryLoading:null, observatoryData:null, observatoryCase:'feature_quality', observatoryTimers:[], selectedProofCase:'feature_quality', proposal:null, traces:[], generatedArtifacts:null, contextFingerprint:null, playTimer:null, missionStarted:0, missionCases:[], selectedMission:'feature_quality', missionId:null, missionEvents:[], missionStream:null, missionTwinState:null, replayTimers:[], memoryLoaded:false, memoryLoading:null, memoryHealth:null, memoryPatient:null,evolution:null,evolutionLoading:null,evolutionGraph:null,evolutionCouncil:null,evolutionMemory:null,evolutionPaths:null,evolutionReplayTimer:null,evolutionFrameIndex:0,helix:null,helixLoading:null,helixGraph:null,helixReplay:null,helixFrameIndex:0,helixTimer:null};
const REPLAY_CACHE_KEY='oncotwin_verified_replay_v1';
const FLOW_GUIDE_STORAGE_KEY='oncotwin.flowGuide.v11.4.completed';
const flowGuideSteps=[
  {view:'mission',label:'Signal',glyph:'01',kicker:'A governed signal enters',title:'Observe before the agents act.',description:'OncoTwin begins with a synthetic research incident and DataHub-grounded context—not an unverified clinical claim.',evidence:['12 research missions','DataHub identity','RL safety state']},
  {view:'memory',label:'Recall',glyph:'02',kicker:'CockroachDB restores durable context',title:'The agent remembers across restarts.',description:'Patient context, clinical events, agent checkpoints, handoffs and approval state rehydrate as one transactional memory bundle.',evidence:['Persistent state','Restart rehydration','Approval history']},
  {view:'evolution',label:'Branch',glyph:'03',kicker:'Specialists reason in tandem',title:'Memory conditions the possible futures.',description:'Five specialist agents compare clonal evolution paths against prior observations, then persist probabilities, votes and evidence receipts.',evidence:['Multi-agent council','Evolution frames','Tamper-evident receipts']},
  {view:'helix',label:'Observe',glyph:'04',kicker:'Progression becomes visible',title:'See mutations pair across generations.',description:'The genome helix turns stored clonal observations into a replayable progression view while keeping projections clearly separated from observed evidence.',evidence:['3D genome helix','Observed vs projected','Generation replay']},
  {view:'ops',label:'Prove',glyph:'05',kicker:'The infrastructure proves itself',title:'Live tools verify the memory layer.',description:'A read-only operations agent uses the official CockroachDB MCP server, ccloud and an Agent Skill, then stores a SHA-256 proof receipt.',evidence:['Official MCP','ccloud CLI','Read-only Agent Skill']},
  {view:'twin',label:'Gate',glyph:'06',kicker:'A human owns the final boundary',title:'Evidence reaches a governed decision—not an automatic treatment.',description:'The flow ends at a human safety gate. Every proposed action stays research-only, auditable and replayable from CockroachDB memory.',evidence:['Human approval','No autonomous care','Verifiable replay']}
];
let flowGuideIndex=0;
...
async function boot(){
  ...
  try{
    ...
    renderAgentPager(); selectAgent(0); renderCode('dbt');renderProofCases();selectProofCase('feature_quality',false);
...
function wireStaticEvents(){
  ...
  $('generateHelixObservation').addEventListener('click',generateHelixObservation);$('playHelixProgression').addEventListener('click',playHelixProgression);$('helixFrameSlider').addEventListener('input',event=>showHelixFrame(Number(event.target.value)));$('helixScenario').addEventListener('change',updateHelixForecast);document.querySelectorAll('[data-helix-mode]').forEach(button=>button.addEventListener('click',()=>selectHelixMode(button.dataset.helixMode)));
...
function scheduleFlowGuide(){
  let completed=false;try{completed=localStorage.getItem(FLOW_GUIDE_STORAGE_KEY)==='true'}catch(_error){}
  if(!completed)setTimeout(()=>openFlowGuide(0),700);
}

function openFlowGuide(index=0){
  const guide=$('flowGuide');if(!guide)return;guide.hidden=false;document.body.classList.add('flow-guide-open');showFlowGuideStep(index);setTimeout(()=>$('flowGuideNext')?.focus(),60);
}

function closeFlowGuide(completed=false){
  const guide=$('flowGuide');if(!guide)return;guide.hidden=true;document.body.classList.remove('flow-guide-open');clearFlowGuideTarget();if(completed){try{localStorage.setItem(FLOW_GUIDE_STORAGE_KEY,'true')}catch(_error){}}$('openFlowGuide')?.focus();
}

function showFlowGuideStep(index){
  flowGuideIndex=Math.max(0,Math.min(flowGuideSteps.length-1,index));const step=flowGuideSteps[flowGuideIndex];
  switchView(step.view);clearFlowGuideTarget();const target=document.querySelector(`.tab[data-view="${step.view}"]`);target?.classList.add('flow-guide-target');
  $('flowGuideStep').textContent=`STEP ${flowGuideIndex+1} OF ${flowGuideSteps.length}`;$('flowGuideView').textContent=step.label.toUpperCase();$('flowGuideGlyph').textContent=step.glyph;$('flowGuideKicker').textContent=step.kicker;$('flowGuideTitle').textContent=step.title;$('flowGuideDescription').textContent=step.description;$('flowGuideEvidence').innerHTML=step.evidence.map(item=>`<span>${escapeHtml(item)}</span>`).join('');$('flowGuideBack').disabled=flowGuideIndex===0;$('flowGuideNext').textContent=flowGuideIndex===flowGuideSteps.length-1?'Enter OncoTwin →':`Next: ${flowGuideSteps[flowGuideIndex+1].label.toLowerCase()} →`;
  $('flowGuideMap').innerHTML=flowGuideSteps.map((item,i)=>`<button type="button" class="${i===flowGuideIndex?'active':''} ${i<flowGuideIndex?'complete':''}" data-flow-step="${i}" aria-label="Step ${i+1}: ${escapeHtml(item.label)}"><i>${i<flowGuideIndex?'✓':String(i+1).padStart(2,'0')}</i><span>${escapeHtml(item.label)}</span></button>`).join('');$('flowGuideMap').querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>showFlowGuideStep(Number(button.dataset.flowStep))));
}

function clearFlowGuideTarget(){document.querySelectorAll('.flow-guide-target').forEach(item=>item.classList.remove('flow-guide-target'))}
function handleFlowGuideKeys(event){const guide=$('flowGuide');if(!guide||guide.hidden)return;if(event.key==='Escape')closeFlowGuide(true);if(event.key==='ArrowRight')flowGuideIndex===flowGuideSteps.length-1?closeFlowGuide(true):showFlowGuideStep(flowGuideIndex+1);if(event.key==='ArrowLeft')showFlowGuideStep(flowGuideIndex-1)}
...
async function ensureEvolutionLab(force=false){
  ...
  state.evolutionLoading=(async()=>{try{
    ...
    const {createEvolutionGraph}=await import('./evolution3d.js?v=11.3.0');state.evolution=createEvolutionGraph($('evolution3d'),graph,{onSelect:renderEvolutionClone,onReady:info=>{$('evolutionRenderer').textContent=`● ${info.version} · ${info.nodes} CLONES · ${info.edges} EDGES`}});
...
async function ensureGenomeHelix(force=false){
  if(state.helix&&!force){state.helix.focus();return state.helix}if(state.helixLoading)return state.helixLoading;
  state.helixLoading=(async()=>{try{$('helixRenderer').textContent='○ REHYDRATING COCKROACHDB HELIX';const [graph,replay]=await Promise.all([fetch('/api/evolution/patients/ONCO-007',{cache:'no-store'}).then(checkJson),fetch('/api/evolution/patients/ONCO-007/memory-replay',{cache:'no-store'}).then(checkJson)]);state.helixGraph=graph;state.helixReplay=replay;if(state.helix)state.helix.dispose();const {createGenomeHelix}=await import('./helix3d.js?v=11.3.0');state.helix=createGenomeHelix($('helix3d'),graph,replay,{onReady:info=>{$('helixRenderer').textContent=`● ${info.version} · ${info.rungs} PAIRS · ${info.hotspots} RUPTURES`},onFrame:renderHelixFrameStats});renderHelixFrames();renderHelixMutationAtlas();renderHelixMemoryImpact();updateHelixForecast();return state.helix}catch(error){$('helixRenderer').textContent='× HELIX MEMORY UNAVAILABLE';$('helixFrameTitle').textContent='No unsupported genome state rendered';$('helixFrameMeta').textContent=error.message;return null}finally{state.helixLoading=null}})();return state.helixLoading;
}

function renderHelixFrames(){const frames=state.helixReplay?.frames||[];$('helixMemoryCount').textContent=`${frames.length} FRAME${frames.length===1?'':'S'}`;$('helixFrameSlider').max=Math.max(0,frames.length-1);$('helixFrameSlider').value=Math.max(0,frames.length-1);$('helixFrameRail').innerHTML=frames.map((frame,index)=>`<button class="helix-frame-card ${index===frames.length-1?'active':''}" data-helix-frame="${index}"><small>GENERATION ${frame.generation}</small><b>${escapeHtml(frame.source_name)}</b><em>${new Date(frame.observed_at).toLocaleDateString()} · ${Object.keys(frame.clone_distribution||{}).length} clone states</em></button>`).join('');$('helixFrameRail').querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>showHelixFrame(Number(button.dataset.helixFrame))));if(frames.length)showHelixFrame(frames.length-1)}

function showHelixFrame(index){const frames=state.helixReplay?.frames||[],frame=frames[index];if(!frame)return;state.helixFrameIndex=index;$('helixFrameSlider').value=index;$('helixProgressLabel').textContent=`G${frame.generation}`;document.querySelectorAll('.helix-frame-card').forEach((button,i)=>button.classList.toggle('active',i===index));state.helix?.setFrame(frame,index?frames[index-1]:null)}

function renderHelixFrameStats({frame,dominance,diversity}){if(!frame)return;$('helixFrameTitle').textContent=`Generation ${frame.generation} · ${Math.round(dominance*100)}% dominant signal`;$('helixFrameMeta').textContent=`${new Date(frame.observed_at).toLocaleDateString()} · ${diversity} detectable clone states · ${frame.source_name}`;$('helixReceipt').textContent=`sha256:${frame.frame_receipt}`;const seen=(state.helixGraph?.mutation_events||[]).filter(event=>new Date(event.event_time)<=new Date(frame.observed_at));$('helixHotspotChips').innerHTML=seen.map(event=>`<i>${escapeHtml(event.gene)} ${escapeHtml(event.alteration)}</i>`).join('')||'<i>Founder sequence baseline</i>'}

function selectHelixMode(mode){document.querySelectorAll('[data-helix-mode]').forEach(button=>button.classList.toggle('active',button.dataset.helixMode===mode));state.helix?.setMode(mode);if(mode==='paired')updateHelixForecast()}

function updateHelixForecast(){const scenario=$('helixScenario').value,path=(state.helixReplay?.saved_paths||[]).find(item=>item.scenario===scenario);if(path)state.helix?.setForecast(path)}

function renderHelixMutationAtlas(){const events=state.helixGraph?.mutation_events||[];$('helixMutationAtlas').innerHTML=events.map((event,index)=>`<div><i>${String(index+1).padStart(2,'0')}</i><span><b>${escapeHtml(event.gene)} · ${escapeHtml(event.alteration)}</b><small>${escapeHtml(event.source_name||'Synthetic assay')} · evidence receipt ${escapeHtml(event.evidence_hash)}</small></span><strong>${Math.round(event.evidence_strength*100)}%</strong></div>`).join('')}

function renderHelixMemoryImpact(){const utility=state.helixReplay?.memory_utility||{};$('helixMemoryImpact').innerHTML=`<div><i>↻</i><span><b>Restart-proof progression</b><small>Every frame returns after application restart.</small></span><strong>${utility.frames_recalled||0}</strong></div><div><i>⌁</i><span><b>Competing futures remembered</b><small>Agent paths remain comparable instead of disappearing.</small></span><strong>${utility.paths_remembered||0}</strong></div><div><i>∆</i><span><b>Forecast divergence retained</b><small>Surprises become evidence for the next council.</small></span><strong>${utility.divergence_events||0}</strong></div><div><i>◫</i><span><b>Longitudinal span</b><small>Observed evolution available to every agent.</small></span><strong>${utility.temporal_span_days||0}d</strong></div>`}

async function generateHelixObservation(){const button=$('generateHelixObservation'),scenario=$('helixScenario').value;button.disabled=true;button.textContent='◌ Pairing forecast with new evidence…';$('helixObservationStatus').textContent='WRITING IMMUTABLE FRAME';try{const result=await fetch('/api/evolution/patients/ONCO-007/observations/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scenario,evidence_strength:.82})}).then(checkJson);$('helixObservationStatus').textContent=result.reconciliation.status.replaceAll('_',' ');$('helixReceipt').textContent=`sha256:${result.frame.frame_receipt}`;$('helixPairing').innerHTML=result.paired_deltas.slice(0,5).map(item=>`<div class="helix-pair ${item.signal}"><span><b>${escapeHtml(item.clone_label)}</b><small>${(item.before*100).toFixed(1)}% → ${(item.after*100).toFixed(1)}%</small></span><strong>${item.delta>=0?'+':''}${(item.delta*100).toFixed(1)}</strong></div>`).join('');state.helixReplay=await fetch('/api/evolution/patients/ONCO-007/memory-replay',{cache:'no-store'}).then(checkJson);renderHelixFrames();renderHelixMemoryImpact();selectHelixMode('paired');button.textContent='✦ Generate another paired observation'}catch(error){$('helixObservationStatus').textContent='STOPPED SAFELY';$('helixPairing').innerHTML=`<p>No frame was invented without memory evidence: ${escapeHtml(error.message)}</p>`;button.textContent='↻ Retry observation'}finally{button.disabled=false}}

function playHelixProgression(){const frames=state.helixReplay?.frames||[],button=$('playHelixProgression');if(!frames.length)return;if(state.helixTimer){clearInterval(state.helixTimer);state.helixTimer=null;button.textContent='▶ Play cancer progression';return}state.helixFrameIndex=-1;button.textContent='Ⅱ Pause progression';const advance=()=>{state.helixFrameIndex=(state.helixFrameIndex+1)%frames.length;showHelixFrame(state.helixFrameIndex);if(state.helixFrameIndex===frames.length-1){clearInterval(state.helixTimer);state.helixTimer=null;button.textContent='↻ Replay progression'}};advance();state.helixTimer=setInterval(advance,1100)}
