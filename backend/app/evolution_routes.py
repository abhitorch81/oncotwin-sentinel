...
from .evolution_repository import evolution_graph, evolution_memory_replay, generate_paired_observation, run_evolution_council, run_evolution_memory_paths
...
class PairedObservationRequest(BaseModel):
    scenario: str = Field(default="resistance_sweep", pattern="^(resistance_sweep|stable_coexistence|plasticity_escape|evidence_pruning)$")
    evidence_strength: float = Field(default=.82, ge=.5, le=1)
...
@router.post("/api/evolution/patients/{synthetic_code}/observations/generate")
def generate_observation(synthetic_code: str, request: PairedObservationRequest) -> dict[str, Any]:
    try: result=generate_paired_observation(synthetic_code,request.scenario,request.evidence_strength)
    except Exception as exc:
        raise HTTPException(503, detail={"message":"A paired observation could not be persisted from the selected memory path.","error_type":type(exc).__name__}) from exc
    if result is None: raise HTTPException(404, detail="Synthetic patient not found")
    return result
