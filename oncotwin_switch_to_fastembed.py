#!/usr/bin/env python3
"""Replace OncoTwin's Gemini Lambda vectorizer with an open-source FastEmbed worker."""
...
HANDLER = r'''# OncoTwin MemoryMesh: AWS Lambda + open-source FastEmbed vectors
from __future__ import annotations
...
def _settings() -> dict[str, str]:
    ...
    required = ("database_url", "shared_token", "tenant_id")
...
def _model() -> TextEmbedding:
    return TextEmbedding(model_name=MODEL_ID, cache_dir=MODEL_CACHE, threads=2)
...
def _embedding(text_value: str, title: str | None, task_type: str) -> list[float]:
    ...
    if task_type == "RETRIEVAL_QUERY":
        model_input = f"query: {text_value}"
    else:
        heading = f"{title.strip()}\n" if title and title.strip() else ""
        model_input = f"passage: {heading}{text_value}"

    native = [float(value) for value in next(_model().embed([model_input]))]
    if len(native) != NATIVE_DIMENSIONS:
        raise RuntimeError(
            f"Expected {NATIVE_DIMENSIONS} native dimensions, received {len(native)}"
        )
    norm = math.sqrt(sum(value * value for value in native))
    if norm == 0:
        raise RuntimeError("Embedding model returned a zero vector")
    normalized = [value / norm for value in native]
    return normalized + [0.0] * (VECTOR_DIMENSIONS - NATIVE_DIMENSIONS)
...
def _health(connection: psycopg.Connection[Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "embedding_provider": "FastEmbed (open source)",
        "embedding_model": MODEL_ID,
        "model_license": "MIT",
        "native_dimensions": NATIVE_DIMENSIONS,
        "storage_dimensions": VECTOR_DIMENSIONS,
...
def _embed_memory(
    ...
    with connection.cursor() as cursor:
        ...
        values = _embedding(content, title, "RETRIEVAL_DOCUMENT")
...
def _semantic_search(
    ...
    values = _embedding(query, None, "RETRIEVAL_QUERY")
    vector_literal = json.dumps(values, separators=(",", ":"))
    with connection.cursor() as cursor:
        cursor.execute(
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        stage = action or "unsupported_action"
        ...
        print(json.dumps({"level": "ERROR", "stage": stage, "error_type": type(exc).__name__}))
        return _response(500, {"ok": False, "error": "Memory vectorizer failed"})
'''


DOCKERFILE = r'''FROM public.ecr.aws/lambda/python:3.12
...
def replace_required(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        raise RuntimeError(f"Could not locate expected {label}; no files were changed")
    return text.replace(old, new)


def main() -> None:
    root = Path.cwd()
    handler = root / "aws/lambda_memory_vectorizer/handler.py"
    dockerfile = root / "aws/lambda_memory_vectorizer/Dockerfile"
    requirements = root / "aws/lambda_memory_vectorizer/requirements.txt"
    deploy = root / "scripts/deploy_lambda_memory_vectorizer.sh"
    required = (handler, dockerfile, requirements, deploy)
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise SystemExit("Run this from the OncoTwin project root. Missing: " + ", ".join(missing))

    deploy_text = deploy.read_text(encoding="utf-8")
    deploy_text = replace_required(
        deploy_text,
...
''',
    ...
    deploy_text = replace_required(
        ...
        "export DATABASE_URL MEMORY_VECTORIZER_TOKEN MEMORY_TENANT_ID",
        "secret export",
    )
    deploy_text = replace_required(
        deploy_text,
    ...
    deploy_text = deploy_text.replace("--timeout 45 --memory-size 1024", "--timeout 90 --memory-size 2048")

    stamp = dt.datetime.now(dt.UTC).strftime("%Y%m%dT%H%M%SZ")
    backup = root / f".fastembed-migration-backup-{stamp}"
    ...
    print("OncoTwin open-source FastEmbed migration applied successfully.")
    print(f"Backup: {backup}")
    print("Model: BAAI/bge-small-en-v1.5 (MIT), 384D padded to VECTOR(1024)")
