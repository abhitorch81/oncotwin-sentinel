    ...
    async def _run(self, mission: dict[str, Any]) -> None:
        ...
        try:
            ...
            if not self.settings.demo_mode and case_id == "feature_quality":
                ...
                title = f"OncoTwin V10 · {case['title']}"
            ...
            self._event(mission, "rl_decision", agent="RL Safety Controller", tool="tabular Q-learning", scene_cue="risk-focus", summary=f"Selected {policy['action']} from the DataHub-grounded twin state.", rl=policy, twin=safe_state.public())
            await asyncio.sleep(0.08)
